...
import type { CaptureQuality } from "../voice/record";
    ...
    <section className="setup-panel" role="dialog" aria-modal="true" aria-labelledby="setup-title" onKeyDown={trapFocus}>
      ...
      <div className="setup-columns">
        <section aria-labelledby="voice-setup-title">
          ...
          <p>{status}</p>
          <label>Microphone <select value={settings.microphoneId} onChange={event => { update({ ...settings, microphoneId: event.target.value }); setHeard(""); setSuccess(false); setQuality(null); }}><option value="">System default</option>{devices.filter(d => d.kind === "audioinput" && d.deviceId).map((device, i) => <option key={device.deviceId} value={device.deviceId}>{device.label || `Microphone ${i + 1}`}</option>)}</select></label>
          <label className="microphone-processing"><input type="checkbox" checked={settings.microphoneProcessing === true} onChange={event => { update({ ...settings, microphoneProcessing: event.target.checked }); setQuality(null); }} /> Browser noise cleanup (try off for EMEET speakerphones)</label>
          <p>Use the input meter to check that sound reaches the app. If it stays low, check the selected input and Ubuntu input volume. The app removes low rumble and normalises speech for recognition; try both browser cleanup settings because the EMEET may already process sound.</p>
          ...
          {voiceReady && <VoiceAnswer key={`${word}-${settings.microphoneId}-${settings.microphoneProcessing}`} constrainRecognition={false} useCorrections={false} choices={[{ value: word === "three" ? "3" : word, label: word, color: null }]} onTranscript={text => { setHeard(text); setSuccess(false); void refreshDevices(); }} onQuality={setQuality} onAnswer={() => setSuccess(true)} />}
          {quality && <p className="microphone-quality" role="status"><strong>Last recording:</strong> channel {quality.selectedChannel} of {quality.channels}; SNR {quality.snrDb.toFixed(1)} dB; speech {Math.round(quality.speechDurationMs)} ms; clipping {quality.clippedPercent.toFixed(2)}%; gain {quality.gain.toFixed(1)}×. {quality.message}</p>}
