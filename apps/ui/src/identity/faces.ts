...
import { BASELINE_FACE_CONFIG, type FaceDetectionConfig } from "./config";
...
export async function detectFaces(canvas: HTMLCanvasElement, observe?: (faces: FaceObservation[]) => void, config: FaceDetectionConfig = BASELINE_FACE_CONFIG): Promise<Face[]> {
  const results = await faceapi.detectAllFaces(canvas, new faceapi.TinyFaceDetectorOptions({ inputSize: config.detectorInputSize, scoreThreshold: config.scoreThreshold })).withFaceLandmarks().withFaceDescriptors();
  ...
  return results.filter(result => result.detection.box.width >= config.minimumBox && result.detection.box.height >= config.minimumBox).map(result => {
    const b = result.detection.box;
