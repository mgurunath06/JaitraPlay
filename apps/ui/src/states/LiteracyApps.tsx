...
import { AIR_LETTERS, airWritingScore, BODY_LETTERS, BUILD_WORDS, focusTokens, matchesBodyLetter, TWO_LETTER_WORDS, wristPoint } from "./literacy";
...
export function WordCardsApp({ onBack }: { onBack: () => void }) {
  const focus=focusTokens(readSettings().weeklyFocus??"").filter(v=>v.length>=2); const words=focus.length?focus:BUILD_WORDS; const [round,setRound]=useState(0),[built,setBuilt]=useState<string[]>([]),[message,setMessage]=useState("Lay out your printed or magnetic letters, then tap the same letters here in order."); const word=words[round%words.length].slice(0,4); const pool=[...new Set([...word,..."aeiost"])].slice(0,6), shift=round%pool.length; const letters=[...pool.slice(shift),...pool.slice(0,shift)]; const correct=built.join("")===word;
